...
export class FitnessAssistantReplyGeneratorService {
    private static readonly CALENDLY_LINK = "calendly.com/jacobberman1995";
    ...
    async generateBookingReply(userInput: string, fromNumber: string){
        ...
        await addCustomerInteraction(
            fromNumber,
            userInput,
            `Sent booking link: ${FitnessAssistantReplyGeneratorService.CALENDLY_LINK}`
        );
        return response;
    }
    ...
    async generateReply(userInput: string, fromNumber: string): Promise<string> {
        ...
        if (intent === "booking") {
            return this.generateBookingReply(userInput, fromNumber)
        }
        ...
        const response = await this.chatService.chatWithHistory(
            fromNumber,
            [`You're a fitness coach's assistant. Additional context from the fitness website: ${additionalContext}\nAnswer questions about rates, availability, and suggest workouts. Keep it short.`],
            [userInput]
        );
