...
export async function updateGymProfile(gymId, profileData) {
    await connectToDatabase();
    const collection = client.db(DB_NAME).collection(COLLECTION_NAME); // Updated collection name
    await collection.updateOne({ _id: gymId }, {
        $set: {
            ...profileData,
...
export async function createGymProfile(gymProfile) {
    await connectToDatabase();
    const collection = client.db(DB_NAME).collection(COLLECTION_NAME); // Updated collection name
    await collection.insertOne(gymProfile);
}
export async function getGymProfileByPhoneNumber(gymPhoneNumber) {
    await connectToDatabase();
    const collection = client.db(DB_NAME).collection(COLLECTION_NAME); // Updated collection name
    return await collection.findOne({ phoneNumber: gymPhoneNumber });
}
// Initialize default gym profile
export async function initializeDefaultGymProfile() {
    const topeiraProfle = {
